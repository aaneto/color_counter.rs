...
fn main() {
    ...
    for (i, region) in space.regions_iter().take(num_regions).enumerate() {
        ...
        for (i, (color, count)) in region.data.iter().take(num_colors).enumerate() {
            println!("Color {}: {} Frequency: {}", i + 1, color.to_rgb_string(), count);
        }
        println!("");
    }
}
